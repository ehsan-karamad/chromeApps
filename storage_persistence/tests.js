...
// complete.
function navigateWebviewToURL(webview, url, callback) {
...
function navigateWebviewToURLAndSetCookie(webview, url, cookie, callback) {
  ...
  navigateWebviewToURL(webview, url, function() {
...
// back its cookies.
function navigateWebviewToURLAndGetCookies(webview, url, callback) {
  var messageHandler = Messaging.GetHandler();
...
// Verifies if |a| and |b| are the same.
...
function createTestToVerifyWebviewSetCookie(webview, url, cookieToSet) {
  return new Testing.Test(webview.id + ' setting cookie ' +
      JSON.stringify(cookieToSet) + ' for ' + url, function(resultCallBack) {
        navigateWebviewToURLAndSetCookie(webview, url, cookieToSet, function() {
          navigateWebviewToURLAndGetCookies(webview, url, function(cookies) {
...
// Creates a test which verifies if a given webview's cookies is what was
// expected.
function createTestToVerifyIfWebviewCookieIsWhatExpected(webview, url,
    expectedCookies) {
  return new Testing.Test(
      'check ' + webview.id + ' has cookies ' + JSON.stringify(expectedCookies),
      function (resultCallBack) {
        navigateWebviewToURLAndGetCookies(webview, url, function(cookies) {
          console.log('Expecting: ' + JSON.stringify(expectedCookies));
          console.log('Received: ' + JSON.stringify(cookies));
          resultCallBack(areSameObjects(cookies, expectedCookies));
        });
      });
}
...
function createPreStoragePersistenceTests(webviews, url) {
  ...
  tests.push(createTestToVerifyIfWebviewCookieIsWhatExpected(webviews[1], url,
        cookie1));
  ...
  tests.push(createTestToVerifyIfWebviewCookieIsWhatExpected(webviews[3], url,
        cookie1));
  ...
  tests.push(createTestToVerifyIfWebviewCookieIsWhatExpected(webviews[5], url,
        cookie2));
...
function createStoragePersistenceTests(webviews, url) {
  ...
  var cookie2 = {'persist1': 'true'};
  var cookie3 = {'persist2': 'true'};
  var expectedCookies = [cookie1, cookie1, cookie1, cookie1, cookie2, cookie2,
  cookie3];
  ...
  for (var index = 0; index < expectedCookies.length; ++index) {
    tests.push(createTestToVerifyIfWebviewCookieIsWhatExpected(webviews[index],
          url, expectedCookies[index]));
  }
...
function getURL(port) {
  return 'http://localhost:' + port + '/page.html';
//      '/extensions/platform_apps/web_view/storage_persistence/page.html';
}

// Runs the test identified in the custom arguments.
function runTest(port, testName) {
  var url = getURL(port);
  var partitionNames = [null, null, 'inmem', 'inmem', 'persist:1', 'persist:1',
  'persist:2'];
  ...
  console.log('Webviews will be loading contents at:' + url);
  test.run(onSuccess, onFailure);
}

window.onload = function() {
  // chrome.test.getConfig(function(config) {
  //  runTest(config.testServer.port, config.customArg);
  // });
}

document.getElementById('preTest').onclick = function() {
  var prePort = document.getElementById('preTestPort').value;
  runTest(prePort, 'PRE_StoragePersistence');
}

document.getElementById('Test').onclick = function() {
  var port = document.getElementById('testPort').value;
  runTest(port, '');
}
